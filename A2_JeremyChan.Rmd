...
## Thresholded over-representation analysis {#method}
...
### g:Profiler querying {#annotation-threshold-comparison}
...
kable(
  ...
  caption = "Table 1. Top 10 enriched pathways from g:Profiler thresholded over-representation analysis using all significantly differentially expressed genes for the TMZRT + UK5099 vs Naive comparison.")

kable(
  filtered_combo_naive_up[1:10, ],
  caption = "Table 2. Top 10 enriched pathways from g:Profiler thresholded over-representation analysis using significantly up-regulated genes for the TMZRT + UK5099 vs Naive comparison.")

kable(
  filtered_combo_naive_down[1:10, ],
  caption = "Table 3. Top 10 enriched pathways from g:Profiler thresholded over-representation analysis using significantly down-regulated genes for the TMZRT + UK5099 vs Naive comparison.")
```
...
Now that we have the parameters set up, we can run the GSEA analysis using the command line software. We will specify the input files, output directory, and parameters such as the minimum and maximum gene set size matching the non-thresholded ORA in the previous section, as well as the number of permutations to run.
...
kable(combo_up[1:10, ], caption = "Table 4. Top 10 enriched pathways in treated samples from GSEA non-thresholded over-representation analysis using all genes for the TMZRT + UK5099 vs Naive comparison.")

kable(naive_up[1:10, ], caption = "Table 5. Top 10 enriched pathways in naive samples from GSEA non-thresholded over-representation analysis using all genes for the TMZRT + UK5099 vs Naive comparison.")
```
