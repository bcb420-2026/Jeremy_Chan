...
In this assignment, we will be analysing the bulk RNA-seq data from GEO accession `GSE253464`, published as part of a study by Martell et al (2025). The authors focuses on the key genes MPC1/MPC2 (mitochondrial pyruvate carrier components), PDHA1/PC (metabolic regulators controlling acetyl-CoA production), and NEUROD1/DCX (neural differentiating genes). The study aims to show that in glioblastoma (GBM) cells that survive both temozolomide and radiation therapy (TMZ-RT), inhibiting MPC through metabolic interventions can reverse and reduce recurrence rates.
...
We normalized the bulk RNA-seq counts using the TMM (Trimmed Mean of M-values) method in `edgeR`. TMM corrects for differences in sequencing depth and compositional bias by trimming genes with extreme fold-changes and extreme expression before estimating scaling factors. This is preferred over methods such as RPKM/FPKM for differential expression because those approaches do not robustly correct for composition bias, potentially creating inaccurate results during the future steps.
...
From the raw counts boxplots (left) in Figure 1, the samples show noticeable differences in their overall count distributions, which is expected because sequencing depth and RNA composition can vary between libraries. After applying TMM normalization (right), the boxplots become more aligned across samples, indicating that scaling factors have adjusted for library size and compositional bias. This will improve comparability between samples and supports using the normalized data for the upcoming downstream analyses, such as MDS visualization and differential expression testing.
...
In Figure 5, the overall pattern shows both up-regulated and down-regulated genes, suggesting that UK-5099 exposure is associated with bi-directional transcriptional changes rather than a one-direction global shift. However, the key genes highlighted from the study’s metabolic and differentiation focus (MPC1, MPC2, PDHA1, PC, NEUROD1, and DCX) do not pass the FDR \< 0.05 threshold in this comparison, indicating that these specific pathways are not strongly altered at the transcript level by UK-5099 alone relative to the therapy-naive state. This suggests that the major effects of MPC inhibition may occur through changes in metabolic activity and protein function rather than large changes in gene expression, and can point to this mono-treatment being ineffective.
...
In Figure 6, there is again a strong differential expression signal, with many genes showing large effect sizes and high statistical significance. Similar to the TMZRT vs. Naive comparison, the neuronal differentiation markers NEUROD1 and DCX remain down-regulated, suggesting continued suppression of differentiation-associated expression in the treated condition. The metabolic genes highlighted from the study (MPC1, MPC2, and PDHA1) do not pass the FDR significance threshold in this comparison, indicating that the combined treatment does not strongly shift these transcripts relative to the naive baseline. Overall, this pattern suggests that the transcriptional differences in this dataset are primarily driven by TMZ-RT exposure, with UK-5099 not producing a significant additional transcript-level shift.

### Heatmap of top genes

Next, I created heatmaps of the top 25 differentially expressed genes to visualise expression patterns across samples and confirm whether samples cluster by treatment condition. In addition to the TMZ-RT vs. Naive comparison, which proved to be the main driver in the volcano plot analysis, I also included a heatmap for TMZ-RT + UK5099 vs TMZ-RT, because this contrast directly tests the study’sclaim that MPC inhibition through UK5099 alters the post-TMZ-RT transcriptional state rather than having strong effects on its own.
...
The heatmap shows that the top 25 genes identified from the TMZ-RT vs. Naive comparison separate the samples into distinct clusters, with naive samples grouping together and treated samples grouping separately. This indicates that TMZ-RT exposure produces a consistent transcriptional shift across replicates, and that the strongest differential expression signals are sufficient to distinguish treatment groups based on global expression patterns.
...
The heatmap shows that the top 25 genes from the TMZRT + UK5099 vs TMZRT comparison separate samples by treatment condition, with replicates clustering together within each group. This indicates that adding UK5099 after TMZRT produces a consistent transcript-level shift relative to TMZRT alone. This contrast is particularly relevant to the study’s main hypothesis, as it directly tests whether MPC inhibition alters the post-TMZRT transcriptional state rather than only shifting expression relative to the therapy-naive baseline.

## Conclusion

In this report, I performed a bulk RNA-seq differential expression analysis using a publicly available GBM dataset (GEO accession number: GSE253464) comparing therapy-naive cells to TMZ-RT exposure and metabolic inhibition with the MPC inhibitor UK5099. Raw counts were quality controlled, filtered, and normalized using TMM, followed by exploratory visualizations (boxplots, density plots, and MDS) to confirm sample structure and guide model design. Differential expression was then tested using `edgeR`’s quasi-likelihood framework, and results were summarized using significance thresholds (raw p-values and FDR), volcano plots, and heatmaps to highlight treatment-driven expression changes. Overall, the results are consistent with the paper’s aim by showing strong TMZ-RT-associated transcriptional shifts and down-regulation of neuronal differentiation markers (NEUROD1 and DCX), while also suggesting that UK5099 has more context-dependent transcript-level effects, particularly when applied after TMZRT.
